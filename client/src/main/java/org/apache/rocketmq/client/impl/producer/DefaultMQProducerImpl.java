...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    /**
     * MQClientInstance 可能在多个 DefaultMQProducerImpl 间共享，依据clientId来区分
     *  mQClientFactory中维护了所有使用到本mQClientFactory的DefaultMQProducerImpl
     *  使用producer_group来区分,如果是同一个组的多个producer（在同一个实例中）那么会造成只注册其中一个到MQClientInstance
     *
     *  他才是真正的实现类
     */
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST://服务刚刚创建
                ...
                //将当前producer信息注册到producer表中，使用producer_group来区分
    ...
    private SendResult sendDefaultImpl(
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {//有topic对应的路由
            ...
            for (; times < timesTotal; times++) {
                ...
                if (mqSelected != null) {//获取Queue成功
                    ...
                    try {
                        ...
                        //6、todo 成功，更新本次调用时间到MQFaultStrategy中
                        ...
                        switch (communicationMode) {
                            ...
                            case SYNC:
                                ...
                                if (sendResult.getSendStatus() != SendStatus.SEND_OK) {
                                    //todo broker存储失败但通信是成功的，所以如果开启了这项配置则发起重试
                    ...
                    } catch (RemotingException e) { // 8、调用接口异常，更新状态到MQFaultStrategy中
                        endTimestamp = System.currentTimeMillis();
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                if (tranMsg != null && Boolean.parseBoolean(tranMsg)) {
                    //提交消息到Broker方法和普通消息调用的是同一个，实现中唯一针对事务消息的修改就是设置了消息的sysFlag，在sendKernelImpl()方法中
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        SendResult sendResult = null;
        //Prepared消息会在消息的自定义属性中添加标识，包含消息类型和发送的ProducerGroup
        ...
        switch (sendResult.getSendStatus()) {
            case SEND_OK: {
                try {
                    ...
                    if (null != localTransactionExecuter) {//默认为空
                        ...
                        //消息发送成功，则回调TransactionListener的实现,执行本地事务，得到本地事务的执行状态。
            ...
            case SLAVE_NOT_AVAILABLE:
                //8、消息持久化失败，则事务回滚
                //如果第4步中prepared消息虽然发送成功，但Broker持久化消息失败，本地事务不会执行，直接回滚
        ...
        try {
            //根据本地事务的执行状态，发送Commit/Rollback消息给Broker
    ...
    public void endTransaction(
        ...
        /**
         * 最后消息是用Oneway的方式提交的，也就是Broker处理无论成功还是失败，Producer不会再做处理。
         * 这里之所以是这个逻辑，是因为RocketMQ之前的版本是有回查逻辑的，当前最新版本把这个逻辑去掉后，
         * 确实大大影响了事务消息的可用性。
         */
