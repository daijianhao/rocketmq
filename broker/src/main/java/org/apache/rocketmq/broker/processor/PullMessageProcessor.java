...
/**
 * Broker通过PullMessageProcessor来处理Consumer的拉取请求
 * <p>
 * 本质pull和push两种模式都是通过Pull来实现的
 */
public class PullMessageProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
        ...
        if (hasSubscriptionFlag) {
            ...
            //4、检查message Model是否一致  没有开启广播模式，却又等于广播，则返回NO PERMISSION
            ...
            //5、检查cosumer未订阅消息，看指定topic是否有订阅信息
        ...
        if (getMessageResult != null) {
            ...
            switch (response.getCode()) {
                case ResponseCode.SUCCESS:
                    ...
                    break;
                case ResponseCode.PULL_NOT_FOUND: //16.4 没有读取到消息，则hold住请求，有新消息时唤醒。等待超时后还是没读到brokerAllowSuspend=false

                    if (brokerAllowSuspend && hasSuspendFlag) {//允许broker挂起请求
                        long pollingTimeMills = suspendTimeoutMillisLong;
                        if (!this.brokerController.getBrokerConfig().isLongPollingEnable()) {//如果没有开启长轮询
                            //使用短轮询超时
                        ...
                        //交给PullRequestHoldService,hold住请求
        ...
        if (storeOffsetEnable) {
            //提交已消费的偏移量
            //todo 这里是按照 host + comsumerGroup + topic + queueId 来记录已拉取的偏移量的 对于单播的方式 一个消费组对于同一个topic应该是只有一个offset应该是没问题的
            //todo 对于广播咋弄
            //todo 记录已成功消费的位置 requestHeader.getCommitOffset() 既表示本次拉取的开始位置，也表示上次拉取的结束位置
